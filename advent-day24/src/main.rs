...
const UNDEF_I32: i32 = 777;

// bool is var
type TCommandTypeVar1Var2Val2 = (u8, usize, usize, i32);

fn parse_var(s: &str) -> (usize, i32) {
    match s {
        "w" => (0, UNDEF_I32),
        "x" => (1, UNDEF_I32),
        "y" => (2, UNDEF_I32),
...
fn get_b_val(var_vec: &Vec<i32>, var_b: &usize, val_b: &i32) -> i32 {
    if *var_b == UNDEF_VAR {
        return *val_b;
    }
...
fn exec_command(var_vec: &mut Vec<i32>, cmd: &TCommandTypeVar1Var2Val2, inp: i32) -> () {
...
fn exec_all_commands2(
    ...
    for cmd in commands {
        if cmd.0 == 1 {
            ...
            let inp: i32 = cc.to_digit(10).unwrap().try_into().unwrap();
            exec_command(var_vec, cmd, inp);
        } else {
            exec_command(var_vec, cmd, 0);
        }
...
fn get_vectors(digit: u8, other: i32) -> Vec<Vec<i32>> {
...
fn main() {
    ...
    // for i in 0..10 {
    //     let j = max - i;
    //     let mut var_vec: Vec<i32> = vec![0; 4];
